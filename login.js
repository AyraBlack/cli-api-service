...
const YOUTUBE_URL = 'https://www.youtube.com'; // Main YouTube URL
...
async function saveCookies(page) {
  ...
  try {
    ...
    const googleCookies = await page.cookies('https://accounts.google.com'); // Get google.com cookies too
    const allCookies = [...ytCookies, ...googleCookies]; // Combine them
    ...
    for (const cookie of allCookies) {
      ...
      cookieStr += [
        ...
        cookie.expires && cookie.expires !== -1 ? Math.round(cookie.expires) : '0', // 0 for session cookies
        cookie.name,
        cookie.value,
      ].join('\t') + '\n';
    }
...
async function tryToLogInToYouTube() {
  ...
  let browser;
  let page; // Define page in the outer scope to access in catch/finally
  ...
  try {
    ...
    try {
      // Check for the specific 2-Step Verification page elements from your screenshot
      ...
      // Attempt to check the "Don't ask again on this device" checkbox
      const checkboxSelector = 'input[type="checkbox"][name="PersistentCookie"], input[type="checkbox"]#PersistentCookie, input[type="checkbox"][aria-label*="Don\'t ask again"]';
      try {
        const checkbox = await page.$(checkboxSelector); // Try to find the checkbox
        if (checkbox) {
          ...
          console.log('Login Robot: "Don\'t ask again" checkbox not found with specific selectors. Trying generic one.');
          // Fallback to a more generic checkbox selector if specific ones fail
          const genericCheckbox = await page.$('input[type="checkbox"]');
          if (genericCheckbox) {
            const isChecked = await page.evaluate(el => el.checked, genericCheckbox);
            if (!isChecked) {
                 console.log('Login Robot: Attempting to check a generic "Don\'t ask again" checkbox...');
                 await genericCheckbox.click({delay: 100});
                 console.log('Login Robot: Generic checkbox click attempted.');
            }
          } else {
            console.log('Login Robot: Generic "Don\'t ask again" checkbox also not found.');
          }
        }
      } catch (cbErr) {
        console.error('Login Robot: Error trying to find/click the checkbox:', cbErr.message);
      }
      ...
      console.log('Login Robot: Specific 2FA phone prompt page not detected quickly, or login proceeded without it. Checking general success indicators...');
      // This block will run if the 2FA page didn't appear or if waitForFunction timed out.
      // We'll try to see if login succeeded anyway (e.g., if 2FA was off or a different type).
      try {
         await page.waitForFunction(
         ...
         await page.screenshot({ path: 'login_post_password_error.png' }); // Screenshot if login indicators fail
    ...
    if (page) { // Check if page is defined before trying to take a screenshot
        try {
            await page.screenshot({ path: 'login_critical_error.png' });
            console.log('Login Robot: Screenshot login_critical_error.png saved due to error.');
        } catch (ssError) {
